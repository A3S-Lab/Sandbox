...
| Process-tree quotas (Linux) | `pids.max`/`memory.max`/`cpu.max` via delegated cgroup v2; probe-gated, fail-closed without delegation | `platform::cgroup` + `policy::resource_quota_integration` |
